...
import { TiltLift } from "@/components/motion/Float";

export function ServiceCard({ service }: { service: Service }) {
  return (
    <TiltLift intensity={2.5}>
      <article className="group grid h-full overflow-hidden rounded-md border border-tide/16 bg-bone/72 shadow-lg shadow-water/14 transition hover:border-tide/32 hover:shadow-2xl hover:shadow-water/24">
        ...
        <div className="grid gap-4 p-5">
          <div>
            ...
            <h3 className="mt-2 font-serif text-3xl leading-tight text-leaf">{service.title}</h3>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link href={`/services/${service.slug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-leaf hover:text-tide">
